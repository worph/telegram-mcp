  ...
  const server = app.listen(port, () => {
    ...
    createDiscoveryResponder({
      ...
      tools: [
        ...
        { name: "ask", description: "Ask the user a question via Telegram and collect their reply — returns a questionId to poll with get_answer", inputSchema: { type: "object", properties: { question: { type: "string" }, chatId: { type: "string" }, timeoutSeconds: { type: "number" }, waitSeconds: { type: "number" } }, required: ["question"] } },
        { name: "get_answer", description: "Get (or long-poll up to 240s for) the user's answer to a question created with ask", inputSchema: { type: "object", properties: { questionId: { type: "string" }, chatId: { type: "string" }, waitSeconds: { type: "number" } } } },
